...
use workspace::{
    ...
    MultiWorkspace, MultiWorkspaceEvent, NextProject, NextProjectGroup, NextThread, Open,
    PreviousProject, PreviousProjectGroup, PreviousThread, SerializedProjectGroupKey,
    ShowFewerThreads, ShowMoreThreads, Sidebar as WorkspaceSidebar, SidebarSide, Toast,
    ToggleWorkspaceSidebar, Workspace, material_panel_backdrop_color,
                ...
                let menu =
                    ContextMenu::build_persistent(window, cx, move |menu, _window, menu_cx| {
                        ...
                        let group_count = multi_workspace
                            .upgrade()
                            .map_or(0, |mw| mw.read(menu_cx).project_group_keys().count());
...
impl Render for Sidebar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let color = cx.theme().colors();
        let bg = material_panel_backdrop_color(
            color
                .title_bar_background
                .blend(color.panel_background.opacity(0.25)),
            cx,
