...
use crate::ui::{
    ...
    ModelSelectorProviderItem, documentation_aside_side,
};
...
impl PickerDelegate for ModelPickerDelegate {
    ...
    fn can_select(&self, ix: usize, _window: &mut Window, _cx: &mut Context<Picker<Self>>) -> bool {
        match self.filtered_entries.get(ix) {
            ...
            Some(ModelPickerEntry::Provider { is_collapsible, .. }) => *is_collapsible,
    ...
    fn confirm(&mut self, _secondary: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        match self.filtered_entries.get(self.selected_index) {
            Some(ModelPickerEntry::Provider { title, .. }) => {
                let title = title.clone();
                self.toggle_group_collapse(&title, window, cx);
            }
            Some(ModelPickerEntry::Model(model_info, _)) => {
            ...
            Some(ModelPickerEntry::Separator(_)) | None => {}
        }
    }
    ...
    match model_list {
        ...
        AgentModelList::Grouped(index_map) => {
            for (group_name, models) in index_map {
                ...
                entries.push(ModelPickerEntry::Provider {
                    title: title.clone(),
                    model_count,
                    is_collapsible,
                    is_collapsed,
...
fn first_selectable_index(entries: &[ModelPickerEntry]) -> usize {
    entries
        .iter()
        .position(|entry| {
            matches!(
                entry,
                ModelPickerEntry::Model(_, _) | ModelPickerEntry::Provider { .. }
            )
        })
        .unwrap_or(0)
}
    ...
    fn toggle_group_collapse(
        ...
        if let Some(models) = self.models.clone() {
            ...
            let provider_index = self.filtered_entries.iter().position(
                |entry| {
                    matches!(entry, ModelPickerEntry::Provider { title, .. } if title == group_name)
                },
            );
    ...
    fn get_entry_labels(entries: &[ModelPickerEntry]) -> Vec<&str> {
        entries
            ...
            .map(|entry| match entry {
                ...
                ModelPickerEntry::Provider { title, .. } => title.as_ref(),
