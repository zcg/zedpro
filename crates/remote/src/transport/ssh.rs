...
/// 解析端口转发参数，例如 `127.0.0.1:8080:localhost:80` 或 IPv6 形式 `[::1]:8080:[::1]:80`。
///
/// 该函数在上层（例如 `remote` crate 顶层与 UI 配置）需要复用，因此保持为公共 API。
pub fn parse_port_forward_spec(spec: &str) -> Result<SshPortForwardOption> {
    ...
    match tokens.len() {
        ...
        _ => anyhow::bail!(
            "Invalid port forward format: {spec} (expected local_port:remote_host:remote_port or local_host:local_port:remote_host:remote_port)"
        ),
    }
}
