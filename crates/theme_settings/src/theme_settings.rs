...
pub fn init(themes_to_load: LoadThemes, cx: &mut App) {
    ...
    let mut prev_window_material_settings = theme::window_material_theme_overrides(cx);
    ...
    cx.observe_global::<SettingsStore>(move |cx| {
        ...
        let window_material_settings = theme::window_material_theme_overrides(cx);
        let theme_overrides = (
            settings.experimental_theme_overrides.clone(),
            settings.theme_overrides.clone(),
        );
...
fn configured_theme(cx: &mut App) -> Arc<Theme> {
    ...
    theme::apply_system_window_material_overrides(theme, cx)
}
