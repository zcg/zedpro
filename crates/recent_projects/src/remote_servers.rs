...
use dev_container::{
    DevContainerBuildStep, DevContainerConfig, DevContainerContext, DevContainerLogLine,
...
use proto::{self, REMOTE_SERVER_PROJECT_ID};
...
pub struct RemoteServerProjects {
    ...
    dev_container_start_in_flight: HashSet<DevContainerKey>,
    dev_container_stop_in_flight: HashSet<DevContainerKey>,
    dev_container_remove_in_flight: HashSet<DevContainerKey>,
...
fn end_devcontainer_in_flight(set: &mut HashSet<DevContainerKey>, key: &DevContainerKey) {
    set.remove(key);
}
...
fn devcontainer_host_from_remote_options(
    ...
    match options {
        ...
        RemoteConnectionOptions::Docker(options) => match &options.host {
            DockerHost::Local => None,
            DockerHost::Wsl(options) => Some(DevContainerHost::Wsl {
                distro_name: options.distro_name.clone(),
                user: options.user.clone(),
...
fn devcontainer_error_hints(message: &str) -> Vec<String> {
    ...
    if message.contains("Failed to process dev container configuration") {
        push_hint(
            "Inspect the live build log above; this can also come from image metadata, container user detection, or other build-time parsing.",
            &mut hints,
        );
        push_hint(
            "If the file itself looks valid, verify the base image can be inspected and built on the Docker or Podman host.",
            &mut hints,
        );
    }
...
impl DevContainerBuildState {
    fn step_index(step: DevContainerBuildStep) -> usize {
        match step {
            DevContainerBuildStep::CheckDocker => 0,
            DevContainerBuildStep::ResolveConfiguration => 1,
            DevContainerBuildStep::FindExistingContainer => 2,
            DevContainerBuildStep::RunInitializeCommand => 3,
            DevContainerBuildStep::BuildContainer => 4,
            DevContainerBuildStep::RunLifecycleScripts => 5,
            DevContainerBuildStep::ReadConfiguration => 6,
        }
    }
    ...
    fn new(window: &mut Window, cx: &mut Context<RemoteServerProjects>) -> Self {
        ...
        let steps = [
            DevContainerUiStep {
                label: SharedString::from("1. Check Docker or Podman"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("2. Resolve dev container config"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("3. Check for existing container"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("4. Run initialize command"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("5. Prepare container"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("6. Run lifecycle scripts"),
                status: DevContainerStepStatus::Pending,
            },
            DevContainerUiStep {
                label: SharedString::from("7. Read config and connect"),
                status: DevContainerStepStatus::Pending,
            },
        ];
    ...
    fn start_step(&mut self, step: DevContainerBuildStep) {
        ...
        self.steps[self.selected_step].status = DevContainerStepStatus::Running;
    }

    fn complete_step(&mut self, step: DevContainerBuildStep) {
        self.selected_step = Self::step_index(step);
        self.steps[self.selected_step].status = DevContainerStepStatus::Completed;
        if step == DevContainerBuildStep::ReadConfiguration {
    ...
    fn fail_step(&mut self, step: DevContainerBuildStep) {
        self.selected_step = Self::step_index(step);
        self.steps[self.selected_step].status = DevContainerStepStatus::Failed;
    ...
    fn append_log_line(
        ...
        let mut formatted = match line.stream {
            ...
            DevContainerLogStream::Stdout => format!("[stdout] {}", line.line),
...
impl RemoteServerProjects {
    ...
    pub fn new_dev_container(
        ...
        if configs.len() > 1 {
            ...
            if let Some(context) = dev_container_context {
                let host_starting_dir = Some(context.project_directory.display().to_string());
                this.open_dev_container_with_context(
                    config,
                    app_state,
                    context,
                    host_starting_dir,
    ...
    fn new_inner(
        ...
        let _project_subscription = None;
        cx.defer_in(window, |this, window, cx| {
            if this._project_subscription.is_some() {
                return;
            }
            ...
            this._project_subscription = Some(cx.subscribe_in(
                &project,
                window,
                |this, _, event: &project::Event, window, cx| {
                    if let project::Event::RemoteDevContainerProgress(progress) = event {
                        this.handle_remote_devcontainer_progress(progress.clone(), window, cx);
                    }
                },
            ));
        });
        ...
        Self {
            ...
            dev_container_start_in_flight: HashSet::new(),
            dev_container_stop_in_flight: HashSet::new(),
            dev_container_remove_in_flight: HashSet::new(),
    ...
    fn view_server_options(
        ...
        self.mode = Mode::ViewServerOptions(match (server_index, connection) {
            ...
            (
                ...
                let mut last_host_project = None;
                    ...
                    .nth(server_index.0)
                {
                        ...
                        && saved.host == host
                    {
                        ...
                        last_host_project = saved.last_host_project.clone();
                ...
                ViewServerOptionsState::DevContainer {
                    connection: DevContainerConnection {
                        ...
                        remote_env: connection.remote_env,
                    },
                    server_index,
                    entries: std::array::from_fn(|_| NavigableEntry::focusable(cx)),
                }
    ...
    fn handle_remote_devcontainer_progress(
        &mut self,
        progress: proto::RemoteDevContainerProgress,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(event) = Self::remote_devcontainer_progress_to_event(progress) else {
            return;
        };
        self.handle_devcontainer_progress_event(event, window, cx);
    }

    fn remote_devcontainer_progress_to_event(
        progress: proto::RemoteDevContainerProgress,
    ) -> Option<DevContainerProgressEvent> {
        let kind = proto::RemoteDevContainerProgressKind::from_i32(progress.kind)?;
        let step = match proto::RemoteDevContainerBuildStep::from_i32(progress.step)? {
            proto::RemoteDevContainerBuildStep::CheckDocker => DevContainerBuildStep::CheckDocker,
            ...
            proto::RemoteDevContainerBuildStep::BuildContainer => {
                DevContainerBuildStep::BuildContainer
            }
        ...
        Some(match kind {
            ...
            proto::RemoteDevContainerProgressKind::StepFailed => {
                DevContainerProgressEvent::StepFailed(step, progress.message)
            }
            proto::RemoteDevContainerProgressKind::LogLine => {
                let stream = match proto::RemoteDevContainerLogStream::from_i32(progress.stream)? {
                    proto::RemoteDevContainerLogStream::Info => DevContainerLogStream::Info,
                    proto::RemoteDevContainerLogStream::Stdout => DevContainerLogStream::Stdout,
                    proto::RemoteDevContainerLogStream::Stderr => DevContainerLogStream::Stderr,
                };
                            ...
                            .child(
                                ListItem::new(("devcontainer-inline-reconnect", ix))
                                    ...
                                    .on_click(cx.listener({
                                        ...
                                        move |this, _, window, cx| {
                                            this.reconnect_dev_container_now(
                                                dev_index,
                                                dev.clone(),
    ...
    fn dev_container_connection_options(
        connection: &DevContainerConnection,
    ) -> Option<DockerConnectionOptions> {
        let options: RemoteConnectionOptions = Connection::DevContainer(connection.clone()).into();
        let RemoteConnectionOptions::Docker(options) = options else {
            return None;
        };
        Some(options)
    ...
    fn reconnect_dev_container_now(
        &mut self,
        index: DevContainerIndex,
        connection: DevContainerConnection,
    ...
    fn stop_dev_container_now(
        ...
        let key = DevContainerKey::from_connection(&connection);
        if !begin_devcontainer_in_flight(&mut self.dev_container_stop_in_flight, &key) {
            return;
        }
    ...
    fn start_dev_container_now(
        ...
        let key = DevContainerKey::from_connection(&connection);
        if !begin_devcontainer_in_flight(&mut self.dev_container_start_in_flight, &key) {
            return;
        }
        ...
        cx.spawn_in(window, async move |_, cx| {
            ...
            remote_servers
                .update_in(cx, |this, window, cx| {
                    end_devcontainer_in_flight(&mut this.dev_container_start_in_flight, &key);
                    match result {
                        Ok(()) => {
                            this.dev_container_statuses.insert(
                                DevContainerKey::from_connection(&connection),
    ...
    fn remove_dev_container_now(
        ...
        let key = DevContainerKey::from_connection(&connection);
        if !begin_devcontainer_in_flight(&mut self.dev_container_remove_in_flight, &key) {
            return;
        }
    ...
    fn init_dev_container_mode(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if configs.len() > 1 {
            ...
            cx.notify();
        } else if let Some((app_state, context, host_starting_dir, project_connection_options)) =
            self.workspace
                .read_with(cx, |workspace, cx| {
                    let app_state = workspace.app_state().clone();
        ...
        {
            let config = configs.into_iter().next();
            self.open_dev_container_with_context(
                config,
                app_state,
                context,
                host_starting_dir,
    ...
    fn open_dev_container(
        ...
        let Some((app_state, context, host_starting_dir, project_connection_options)) = self
            .workspace
            .read_with(cx, |workspace, cx| {
                let app_state = workspace.app_state().clone();
                let context = DevContainerContext::from_workspace(workspace, cx)?;
    ...
    fn open_dev_container_with_context(
        &self,
        config: Option<DevContainerConfig>,
        app_state: Arc<AppState>,
        ...
        cx.spawn_in(window, async move |entity, cx| {
            ...
            let progress_callback: DevContainerProgressCallback = Arc::new(move |event| {
                let _ = progress_tx.unbounded_send(event);
            });
            let environment = context.environment(cx).await;
            let start_result = if Self::should_start_dev_container_on_remote_host(
                project_connection_options.as_ref(),
                remote_client.is_some(),
            ) {
                Self::start_dev_container_on_remote_host(
                    remote_client.clone().expect("remote client checked above"),
                    &context,
                    config.clone(),
                    environment,
                    cx,
                ...
                let context = Self::normalize_devcontainer_project_directory(
                    context,
                    project_connection_options.as_ref(),
                )
                .await;
            ...
            let (mut dev_connection, starting_dir) = match start_result {
                Ok((c, s)) => (c, s),
                Err(e) => {
            ...
            match result {
                Ok(_) => {
                    progress_callback(DevContainerProgressEvent::StepCompleted(
                        DevContainerBuildStep::ReadConfiguration,
                    ));
                    entity
                ...
                Err(e) => {
                    log::error!("Failed to connect: {e:#}");
                    let message = e.to_string();
                    progress_callback(DevContainerProgressEvent::StepFailed(
                        DevContainerBuildStep::ReadConfiguration,
                        message.clone(),
                    ));
                    entity
    ...
    fn should_start_dev_container_on_remote_host(
        project_connection_options: Option<&RemoteConnectionOptions>,
        has_remote_client: bool,
    ) -> bool {
        has_remote_client
            && matches!(
                project_connection_options,
                Some(RemoteConnectionOptions::Ssh(_) | RemoteConnectionOptions::Wsl(_))
            )
    }

    async fn start_dev_container_on_remote_host(
        remote_client: Entity<RemoteClient>,
        context: &DevContainerContext,
        config: Option<DevContainerConfig>,
        environment: HashMap<String, String>,
        cx: &mut AsyncApp,
    ) -> Result<(DevContainerConnection, String), dev_container::DevContainerError> {
        let proto = remote_client.read_with(cx, |client, _| client.proto_client());
        let response = proto
            .request(proto::StartRemoteDevContainer {
                project_id: REMOTE_SERVER_PROJECT_ID,
                project_directory: context.project_directory.to_string_lossy().into_owned(),
                use_podman: context.use_podman,
                config: config.map(|config| proto::RemoteDevContainerConfig {
                    name: config.name,
            ...
            .await
            .map_err(|err| {
                dev_container::DevContainerError::DevContainerUpFailed(err.to_string())
            })?;

        let connection = response.connection.ok_or_else(|| {
            dev_container::DevContainerError::DevContainerUpFailed(
                "remote host did not return a dev container connection".to_string(),
            )
        })?;
    ...
    async fn normalize_devcontainer_project_directory(
        mut context: DevContainerContext,
        project_connection_options: Option<&RemoteConnectionOptions>,
    ) -> DevContainerContext {
        #[cfg(target_os = "windows")]
        {
            let Some(wsl_options) = Self::devcontainer_wsl_host(project_connection_options) else {
                return context;
            };

            if !context.project_directory.to_string_lossy().starts_with('/') {
                return context;
            }

            match wsl_path_to_windows_path(&wsl_options, context.project_directory.as_ref()).await {
                Ok(path) => {
                    context.project_directory = path.into();
                }
                Err(err) => {
                    log::warn!("Failed to convert WSL project path to Windows path: {err:#}");
                }
            }
        }
    ...
    fn devcontainer_wsl_host(
        project_connection_options: Option<&RemoteConnectionOptions>,
    ) -> Option<WslConnectionOptions> {
        match project_connection_options? {
            RemoteConnectionOptions::Wsl(options) => Some(options.clone()),
            RemoteConnectionOptions::Docker(options) => match &options.host {
                DockerHost::Wsl(options) => Some(options.clone()),
                _ => None,
            },
            _ => None,
        }
