...
use dap::{
    StackFrame,
    requests::{Scopes, Source as DapSourceRequest, StackTrace, Threads},
};
...
#[gpui::test]
async fn test_source_reference_is_preferred_over_source_path(
    executor: BackgroundExecutor,
    cx: &mut TestAppContext,
) {
    init_test(cx);
    ...
    let local_file_content = r#"
        // local file should not be opened when sourceReference exists.
        console.log('local file');
    "#
    .unindent();
    let source_reference_content = r#"
        using System;

        public static class GuardAgainstNullExtensions {
            public static void GuardAgainstNull(this object value) {
    ...
    client.on_request::<DapSourceRequest, _>({
        let source_reference_content = source_reference_content.clone();
        move |_, args| {
            assert_eq!(14, args.source_reference);
            Ok(dap::SourceResponse {
                content: source_reference_content.clone(),
                mime_type: Some("text/x-csharp".into()),
            })
        }
    });
    ...
    let stack_frames = vec![StackFrame {
        ...
        name: "Stack Frame 1".into(),
        source: Some(dap::Source {
            name: Some("GuardAgainstNullExtensions.cs".into()),
            path: Some(path!("/project/src/test.js").into()),
            source_reference: Some(14),
            presentation_hint: None,
            origin: None,
            sources: None,
            adapter_data: None,
    ...
    workspace
        .update(cx, |workspace, window, cx| {
            ...
            assert_eq!(source_reference_content, editors[0].read(cx).text(cx));
            assert!(
                editors[0]
                    .update(cx, |editor, cx| editor.project_path(cx))
                    .is_none(),
                "sourceReference should open a virtual document, not a local path file"
            );
