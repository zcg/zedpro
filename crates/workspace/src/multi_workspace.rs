...
use crate::{
    AppState, CloseIntent, CloseWindow, DockPosition, Event as WorkspaceEvent, Item, ModalView,
    OpenMode, Panel, Toast, Workspace, WorkspaceId, WorkspaceSettings, client_side_decorations,
...
pub trait Sidebar: Focusable + Render + EventEmitter<SidebarEvent> + Sized {
    ...
    /// Moves the active project group to a new window.
...
impl MultiWorkspace {
    ...
    pub fn remove_project_group(
        ...
        self.remove(
            ...
            move |this, window, cx| {
                ...
                Task::ready(Ok(this.new_empty_workspace(window, cx)))
            },
            window,
            cx,
        )
    ...
    fn fallback_workspace_after_removal(
        &mut self,
        removed_workspaces: &[Entity<Workspace>],
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Result<Entity<Workspace>>> {
        if let Some(workspace) = self
            ...
            .find(|candidate| {
                !removed_workspaces
                    .iter()
                    .any(|removed| removed == *candidate)
            })
            .cloned()
        {
            Task::ready(Ok(workspace))
        } else {
            Task::ready(Ok(self.new_empty_workspace(window, cx)))
        }
    }
        ...
        self._create_task = Some(cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| match result {
                ...
                Err(error) => {
                    ...
                    if let Some(workspace) = weak_workspace.upgrade() {
                        let removed_workspaces = vec![workspace];
                        let fallback_workspaces = removed_workspaces.clone();
                        this.remove(
                            removed_workspaces,
                            move |this, window, cx| {
                                this.fallback_workspace_after_removal(
                                    &fallback_workspaces,
                                    window,
                                    cx,
                                )
    ...
    pub fn move_workspace_to_new_window(
        ...
        let removed_workspaces = vec![workspace.clone()];
        let fallback_workspaces = removed_workspaces.clone();
        let remove_task = self.remove(
            removed_workspaces,
            move |this, window, cx| {
                this.fallback_workspace_after_removal(&fallback_workspaces, window, cx)
            },
            window,
            cx,
        );
        ...
        cx.spawn_in(window, async move |_this, cx| {
            ...
            cx.update(|_, cx| {
                ...
                let Ok(window) = cx.open_window(options, |window, cx| {
                    cx.new(|cx| MultiWorkspace::new(workspace.clone(), window, cx))
                }) else {
                    return;
                };
    ...
    pub fn move_project_group_to_new_window(
        ...
        let app_state = workspaces[0].read(cx).app_state().clone();
        let sidebar_was_open = self.sidebar_open;
        let removed_workspaces = workspaces.clone();
        let fallback_workspaces = removed_workspaces.clone();
        let remove_task = self.remove(
            workspaces,
            move |this, window, cx| {
                if let Some(neighbor_key) = neighbor_key.clone() {
                    return this.find_or_create_local_workspace(
                        neighbor_key.path_list().clone(),
                        window,
                        cx,
        ...
        cx.spawn_in(window, async move |_this, cx| {
            ...
            cx.update(|_, cx| {
                ...
                let first = removed_workspaces[0].clone();
                let rest = removed_workspaces[1..].to_vec();

                let Ok(new_window) = cx.open_window(options, |window, cx| {
                    cx.new(|cx| MultiWorkspace::new(first.clone(), window, cx))
                }) else {
                    return;
                };
