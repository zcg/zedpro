...
impl<T> PipelineState<T> {
    fn new(
        ...
        let vertex = {
            let raw_shader = RawShaderBytes::for_direct3d11(shader_module, ShaderTarget::Vertex);
            create_vertex_shader(device, raw_shader.as_bytes())?
        };
        let fragment = {
            let raw_shader = RawShaderBytes::for_direct3d11(shader_module, ShaderTarget::Fragment);
...
pub(crate) mod shader_resources {
    ...
    impl<'t> RawShaderBytes<'t> {
        pub(crate) fn for_direct3d11(module: ShaderModule, target: ShaderTarget) -> Self {
            Self::from_bytes(ShaderBackend::Direct3d11, module, target)
        }

        pub(crate) fn for_direct3d12(module: ShaderModule, target: ShaderTarget) -> Self {
            Self::from_bytes(ShaderBackend::Direct3d12, module, target)
        }
        ...
        fn from_bytes(backend: ShaderBackend, module: ShaderModule, target: ShaderTarget) -> Self {
            let bytes = match backend {
                ShaderBackend::Direct3d11 => compiled_d3d11::shader_bytes(module, target),
                ShaderBackend::Direct3d12 => compiled_d3d12::shader_bytes(module, target),
            };
            Self { inner: bytes }
        }
    }
    ...
    mod compiled_d3d11 {
        use super::{ShaderModule, ShaderTarget};

        include!(concat!(env!("OUT_DIR"), "/shaders_fxc_bytes.rs"));

        pub(super) fn shader_bytes(module: ShaderModule, target: ShaderTarget) -> &'static [u8] {
            match module {
    ...
    mod compiled_d3d12 {
        ...
        include!(concat!(env!("OUT_DIR"), "/shaders_dxc_bytes.rs"));
